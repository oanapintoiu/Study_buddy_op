...
const GroupController = {
  ...
  Create: async (req, res) => {
    upload.single("groupCard")(req, res, async (err) => {
      ...
      try {
        let { name, category, subcategory, level, partySize, groupType } =
          req.body;
        ...
        if (req.file) {
          group.groupCard = `${req.protocol}://${req.get("host")}/avatars/${
            req.file.filename
          }`;
        }
        await group.save();
  ...
  CreatePost: async (req, res) => {
    try {
      ...
      const userProjection = await User.findOne(
        { username: user },
        "_id"
      ).exec();
